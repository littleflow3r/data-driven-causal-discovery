...
from algs.dagma_wrapper import dagma_nonlinear_wrapper, dagma_linear_wrapper
import numpy as np
...
def data_sampling(df, size, seed, sample_method):
  ...
  if sample_method == 'systematic':
    ...
    df['cluster'] = np.random.randint(0, 10, size=len(df))  # Assigning clusters (10 clusters: 0-9)
    ...
    df_sampled = df.groupby('cluster', group_keys=False).apply(lambda x: x.sample(n=min(len(x), 10), random_state=seed))
    df_sampled = df_sampled.drop(columns=['cluster'])
...
if args.alg == 'stat_all':
  ...
  predicted_adj_mat_bfsobs = llm_bfs(VAR_NAMES_AND_DESC[args.dataset], args.dataset, df, args.temp, include_statistics=False, include_obs=True)
  ...
  # predicted_adj_mat_llmpobs = llm_pairwise(VAR_NAMES_AND_DESC[args.dataset], prompts[args.dataset], df, args.temp, include_statistics=False, include_obs=True)
  ...
  write_metric(adj_mat, predicted_adj_mat_bfsobs, args.logdir, args.dataset, args.n_samples, f'llm{args.temp}{args.sample_method}_bfs_with_stat')

  print (f'\nRunning LLM pairwise with statistics...')
  predicted_adj_mat_llmpobs = llm_pairwise(VAR_NAMES_AND_DESC[args.dataset], prompts[args.dataset], df, args.temp, include_statistics=True, include_obs=False)
  write_metric(adj_mat, predicted_adj_mat_llmpobs, args.logdir, args.dataset, args.n_samples, f'llm{args.temp}{args.sample_method}_pairwise_with_stat')
  ...
  predicted_adj_mat_llmp = llm_pairwise(VAR_NAMES_AND_DESC[args.dataset], prompts[args.dataset], df, args.temp)
